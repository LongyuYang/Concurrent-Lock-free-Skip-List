...
class lock_free_skip_list : public skip_list<_Key, _Val> {
  private:
    ...
    retry:
        ...
        for (ssize_t i = this->max_level - 1; i >= 0; i--) {
            ...
            if (left_next != right &&
                CAS(&left->next[i], left_next, right) != left_next)
                goto retry;
  ...
  public:
    ...
    virtual _Val get(_Key key) {
        _node **left_list =
            (_node **)GC_MALLOC(this->max_level * sizeof(_node *));
        _node **right_list =
            (_node **)GC_MALLOC(this->max_level * sizeof(_node *));
    ...
    retry:
        ...
        if (CAS(&left_list[0]->next[0], right_list[0], new_node) !=
            right_list[0]) {
            goto retry;
        }
        ...
        for (size_t i = 1; i < new_level; i++) {
            ...
            while (true) {
                ...
                if (new_next != right &&
                    CAS(&new_node->next[i], unmark(new_next), right) !=
                        unmark(new_next))
                    break;
                if (*right == key) {
                    right = unmark(right->next[i]);
                }
                if (CAS(&left_list[i]->next[i], right, new_node) == right)
                    break;
